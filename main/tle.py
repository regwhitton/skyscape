...
def build_tle_dtype():
    ...
    return np.dtype(
        [
            ...
            (EPOCH_YEAR, cltypes.int),
            (EPOCH_DAYS, cltypes.double),
            (FIRST_DERIVATIVE_OF_MEAN_MOTION, cltypes.double),
            (SECOND_DERIVATIVE_OF_MEAN_MOTION, cltypes.double),
            (BSTAR_DRAG_COEFFICIENT, cltypes.double),
            (ORBITAL_INCLINATION, cltypes.double),
            (RIGHT_ASCENSION, cltypes.double),
            (ORBITAL_ECCENTRICITY, cltypes.double),
            (ARG_OF_PERIGEE, cltypes.double),
            (MEAN_ANOMALY, cltypes.double),
            (MEAN_MOTION, cltypes.double),
            (REVOLUTION_NUMBER_AT_EPOCH, cltypes.int),
        ]
    )
...
def parse_tle(tle_name, line1, line2):
    ...
    second_derivative_of_mean_motion = (
        line1[44] + "0." + line1[45:50] + "E" + line1[50:52]
    )
    ...
    tle = {}
    tle[SATELLITE_NUMBER] = np.array(list(sat_number.encode()), dtype=cltypes.char)
    tle[EPOCH_YEAR] = cltypes.int(epoch_year)
    tle[EPOCH_DAYS] = cltypes.double(epoch_days)
    tle[FIRST_DERIVATIVE_OF_MEAN_MOTION] = cltypes.double(
        first_derivative_of_mean_motion
    )
    tle[SECOND_DERIVATIVE_OF_MEAN_MOTION] = cltypes.double(
        second_derivative_of_mean_motion
    )
    tle[BSTAR_DRAG_COEFFICIENT] = cltypes.double(bstar_drag_coefficient)

    tle[ORBITAL_INCLINATION] = cltypes.double(orbital_inclination)
    tle[RIGHT_ASCENSION] = cltypes.double(right_ascension)
    tle[ORBITAL_ECCENTRICITY] = cltypes.double(orbital_eccentricity)
    tle[ARG_OF_PERIGEE] = cltypes.double(arg_of_perigee)
    tle[MEAN_ANOMALY] = cltypes.double(mean_anomaly)
    tle[MEAN_MOTION] = cltypes.double(mean_motion)
    tle[REVOLUTION_NUMBER_AT_EPOCH] = cltypes.int(revolution_number_at_epoch)
    return tle
...
def _validate_tle(tle_name, line1, line2):
    ...
    if line1[2:7] != line2[2:7]:
        raise Exception(
            "TLE {} does not have the same satellite number on both lines".format(
                tle_name
            )
        )
...
def _validate_line(tle_name, line_number, line, regexp):
    if regexp.fullmatch(line) is None:
        raise Exception(
            "TLE {} line {} has incorrect format".format(tle_name, line_number)
        )
    if _check_digit(line) != _calc_expected_check_digit(line):
        msg = "TLE {} line {} does not have correct check digit. Found {} expected {}".format(
            tle_name, line_number, _check_digit(line), _calc_expected_check_digit(line)
        )
        # raise Exception(msg)
