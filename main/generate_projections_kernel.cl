...
void project(__write_only image3d_t image, int frame, double range, double azimuth, double elevation, int image_width, int image_height, char* satnum);
...
{        
    ...
    elsetrec satrec = satrec_array[get_global_id(0)];
    ...
    if (first_visible) {
        project(image, 0, first_range, first_azimuth, first_elevation, image_width, image_height, satrec.satnum);
    }
    for (int sec = 1; sec < n_jtimes-1; sec++) {
        bool visible = calc_razel(&satrec, &jtimes[sec], &range, &azimuth, &elevation);
        if (visible) {
            project(image, sec, range, azimuth, elevation, image_width, image_height, satrec.satnum);
        }
    }
    if (last_visible && !only_one_point) {
        project(image, n_jtimes-1, last_range, last_azimuth, last_elevation, image_width, image_height, satrec.satnum);
    }
}
