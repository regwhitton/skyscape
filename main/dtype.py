...
def to_opencl_dtype(
    opencl_device, dtype, struct_name, source_filename, include_files=None
):

    aligned_dtype, c_decl = tools.match_dtype_to_c_struct(
        opencl_device, struct_name, dtype
    )

    registered_dtype = tools.get_or_register_dtype(struct_name, aligned_dtype)
