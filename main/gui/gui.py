...
def gui_display_images(queue, flags, opencl_ctx):
    ...
    title_txt = sg.Text("Skyspace", font=("Calibri", title_font_size), expand_x=True)
    time_txt = sg.Text(key="time", font=font, expand_x=True)
    norad_id_txt = sg.Text(key="norad_id", font=font, expand_x=False)
    name_txt = sg.Text(key="name", font=font, expand_x=True)
    altitude_txt = sg.Text(key="altitude", font=font, expand_x=True)
    longitude_txt = sg.Text(key="longitude", font=font, expand_x=True)
    latitude_txt = sg.Text(key="latitude", font=font, expand_x=True)
    velocity_txt = sg.Text(key="velocity", font=font, expand_x=True)
    tags_txt = sg.Text(key="tags", font=font, expand_x=True)
    copy_but = sg.Button(
        "copy",
    ...
    while not flags.exiting:
        ...
        if latlong_calc is None:
            latlong_calc = _LatlongCalculator(opencl_ctx, flags.satrec_buf)

        image = Image.fromarray(frame)
        if tracked_sat_found:
            tracked_sat_found, tracked_sat_y, tracked_sat_x = search_for_sat(
                sat_idx, info, tracked_sat_y, tracked_sat_x, 150
            )
            if tracked_sat_found:
                ImageDraw.Draw(image).circle(
                    (
                        tracked_sat_x,
                        tracked_sat_y,
                    ),
        ...
        while waiting_for_frame_time:
            ...
            if ftime <= now:
                ...
                millisec_wait = (
                    wait_delta.seconds * 1000 + wait_delta.microseconds / 1000
                )
        ...
        if pos.clicked:
            ...
            if not found:
                ...
                window["name"].update(
                    value="Name:        {}".format(sat_info[sat_idx]["name"])
                )
        ...
        if not np.isnan(latlong[0]):
            window["altitude"].update(
                value="Altitude:     {:-3,.0f} km".format(latlong[1])
            )
            window["longitude"].update(value="Longitude:  {:-3,.2f}".format(latlong[4]))
            window["latitude"].update(
                value="Latitude:     {:-3,.2f}".format(latlong[3])
            )
            window["velocity"].update(
                value="Velocity:      {:-3,.1f} km/s".format(latlong[0])
            )
        else:
            window["altitude"].update("")
...
def search_for_sat(sat_idx, info, last_known_y, last_known_x, max_search_distance):
    ...
    height, width = info.shape
    return search_box(
        matcher, last_known_y, last_known_x, height, width, max_search_distance
    )
...
class _LatlongCalculator:
    def __init__(self, opencl_ctx, satrec_buf):
        ...
        program = cl.Program(opencl_ctx, '#include "calc_latlong_kernel.cl"').build(
        ...
        mf = cl.mem_flags
        self.output_size = 5
        dummy_output_array = np.empty([self.output_size], cltypes.double)
        self.output_buf = cl.Buffer(
            opencl_ctx,
            mf.READ_WRITE | mf.HOST_READ_ONLY,
    ...
    def calculate_latlong(self, time, sat_idx):
        ...
        p_year = cltypes.int(time.year)
        p_month = cltypes.int(time.month)
        p_day = cltypes.int(time.day)
        p_hour = cltypes.int(time.hour)
        p_min = cltypes.int(time.minute)
        p_sec = cltypes.double(time.second)
        ...
        kernel_event = cl.enqueue_nd_range_kernel(
            self.opencl_queue, self.kernel, (1,), None
        )
